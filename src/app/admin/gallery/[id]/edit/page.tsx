...
export default function EditGalleryPhotoPage({ params }: PageProps) {
    ...
    async function fetchPhoto() {
      try {
        ...
        if (res.ok) {
          ...
          setCategory(data.category || "international");
          setImage(data.image || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        setError(data.error || "Failed to save photo changes.");
      }
    } catch (err) {
    ...
    <div className="max-w-xl mx-auto space-y-6 select-none animate-fadeIn">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 mt-0.5">
          Editing parameters for photo ID: <strong className="text-slate-600 font-mono">{id}</strong>
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-slate-100 rounded-3xl p-6 md:p-8 shadow-xs space-y-6">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-[10px] font-black uppercase tracking-wider text-slate-400">
            Change Photograph <span className="text-rose-500">*</span>
          </label>
