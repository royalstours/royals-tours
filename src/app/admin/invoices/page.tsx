...
export default function AdminInvoicesPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setInvoices((prev) => prev.filter((b) => b._id !== deleteId));
        setDeleteId(null);
      } else {
        alert("Failed to delete client invoice.");
      }
    } catch (err) {
      alert("Error deleting client invoice.");
    } finally {
      setDeleting(false);
    }
  };
  ...
  const filtered = invoices.filter(
    (b) =>
      ...
      b.billedTo?.name.toLowerCase().includes(search.toLowerCase()) ||
      b.bookingReference.toLowerCase().includes(search.toLowerCase()) ||
      b.tripDetails?.destination.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="space-y-6 select-none animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs font-bold text-slate-400 mt-1">
            Create, manage, print and download customer booking invoices.
          </p>
        </div>
        <Link
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xs">
        ...
        <div className="flex items-center gap-4 text-xs font-bold text-slate-500">
          <span>Total invoices: <strong className="text-slate-800">{filtered.length}</strong></span>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-12 text-center border border-slate-100 shadow-xs">
          ...
          <h3 className="font-heading font-black text-lg text-slate-950 uppercase">No Invoices Found</h3>
          <p className="text-xs font-semibold text-slate-400 mt-1 mb-4">
            Try adjusting your search query or create a new client invoice.
          </p>
          <Link
            href="/admin/invoices/new"
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100 text-[10px] font-black uppercase text-slate-400 tracking-wider">
                  ...
                  <th className="py-4 px-4">Billed To</th>
                ...
                {filtered.map((b) => (
                  <tr key={b._id} className="hover:bg-slate-50/50">
                    <td className="py-4 px-6 font-bold text-slate-900 font-mono">
                      {b.invoiceNumber}
                      <span className="block text-[8px] text-slate-400 font-bold uppercase mt-0.5">Ref: {b.bookingReference}</span>
                    </td>
                    <td className="py-4 px-4">
                      <span className="font-bold text-slate-900 block">{b.billedTo?.name}</span>
                      <span className="text-[10px] text-slate-400">{b.billedTo?.phone}</span>
                    </td>
                    <td className="py-4 px-4 text-right font-mono font-bold text-slate-800">
                      ₹ {b.priceSummary?.totalAmount?.toLocaleString("en-IN")}
                    </td>
                    <td className="py-4 px-4 text-right font-mono font-bold text-rose-500">
                      ₹ {b.priceSummary?.balanceDue?.toLocaleString("en-IN")}
                    </td>
                    <td className="py-4 px-4 text-center">
                      {b.paymentStatus === "Paid" && (
                        <span className="bg-emerald-50 text-emerald-650 border border-emerald-100 px-2 py-0.5 rounded text-[9px] font-black uppercase">
                          Paid
                        </span>
                      )}
                      {b.paymentStatus === "Partially Paid" && (
                        <span className="bg-amber-50 text-amber-650 border border-amber-100 px-2 py-0.5 rounded text-[9px] font-black uppercase">
                          Partial
                        </span>
                      )}
                      ...
                      {b.paymentStatus === "Pending" && (
                        <span className="bg-rose-50 text-rose-650 border border-rose-100 px-2 py-0.5 rounded text-[9px] font-black uppercase">
                          Pending
                        </span>
                      )}
                    </td>
                    <td className="py-4 px-6 text-center space-x-3">
                      <Link
                        href={`/admin/invoices/${b._id}/view`}
                        className="text-emerald-650 hover:text-emerald-555 font-bold"
                      >
                        📄 View &amp; PDF
                      </Link>
                      <Link
                        href={`/admin/invoices/${b._id}/edit`}
                        className="text-amber-650 hover:text-amber-555 font-bold"
                      >
                        Edit
                      </Link>
                      <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 backdrop-blur-xs p-4">
          <div className="bg-white rounded-3xl p-6 md:p-8 max-w-sm w-full border border-slate-100 shadow-2xl animate-dialog">
            ...
            <p className="text-xs text-slate-500 mt-2">
              Are you sure you want to delete invoice <strong>{deleteRef}</strong>? This action will remove the record from MongoDB permanently.
            </p>
