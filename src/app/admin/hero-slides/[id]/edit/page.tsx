...
export default function EditHeroSlidePage({ params }: PageProps) {
    ...
    async function fetchSlide() {
      try {
        ...
        if (res.ok) {
          ...
          setError("Failed to fetch slide details.");
        }
      } catch (err) {
        setError("Error loading slide details.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.error || "Failed to save slide changes.");
      }
    } catch (err) {
